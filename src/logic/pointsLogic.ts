...
export const calculatePointsFromResources = (resourceCost: number): number => {
  return Math.floor(resourceCost / 1000 / 1000);
};
 ...
 * @returns 积分数
 */
export const calculateBuildingPoints = (
 ...
 * @returns 积分数
 */
export const calculateTechnologyPoints = (
 ...
 * @returns 积分数
 */
export const calculateShipPoints = (
  shipType: ShipType,
  quantity: number
): number => {
 ...
 * @returns 积分数
 */
export const calculateDefensePoints = (
...
export const calculateTotalPlayerPoints = (player: Player): number => {
  ...
  for (const planet of player.planets) {
    for (const [buildingType, level] of Object.entries(planet.buildings)) {
      if (level > 0) {
        totalPoints += calculateBuildingPoints(
    ...
    for (const [defenseType, quantity] of Object.entries(planet.defense)) {
      if (quantity > 0) {
        totalPoints += calculateDefensePoints(
          defenseType as DefenseType,
          quantity
        );
      }
    }
  ...
  for (const [technologyType, level] of Object.entries(player.technologies)) {
    if (level > 0) {
      totalPoints += calculateTechnologyPoints(
